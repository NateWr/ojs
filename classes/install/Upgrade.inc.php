...
class Upgrade extends Installer
{
     ...
     * @return boolean
     */
    public function removeCancelledReviewAssignments()
    {
     ...
     * @return boolean
     */
    public function repairKeywordsAndSubjects()
    {
        $request = Application::get()->getRequest();
        ...
        foreach ($result as $row) {
            ...
            if (!empty($keywordsToAdd)) {
                $submissionKeywordDao->insertKeywords([$locale => $keywordsToAdd], $submissionId, false, ASSOC_TYPE_SUBMISSION);
            }
        ...
        foreach ($result as $row) {
            ...
            $newSubjects = [];
            $newSubjectsArray = $submissionSubjectDao->getSubjects($submissionId, [$locale], ASSOC_TYPE_SUBMISSION);
            ...
            if (!empty($subjectsToAdd)) {
                $submissionSubjectDao->insertSubjects([$locale => $subjectsToAdd], $submissionId, false, ASSOC_TYPE_SUBMISSION);
            }
        }
     ...
     * @return boolean
     */
    public function enabledSitePlugins()
    {
        ...
        foreach ($allPlugins as $plugin) {
            if ($plugin->isSitePlugin()) {
                ...
                if ($pluginName != 'customblockmanagerplugin') {
                    $result = $pluginSettings->update('DELETE FROM plugin_settings WHERE plugin_name = ? AND setting_name = \'enabled\' AND context_id <> 0', [$pluginName]);
                }
            }
        }
     ...
     * @return boolean
     */
    public function fixGenreIdInFileNames()
    {
        $journalDao = DAORegistry::getDAO('JournalDAO'); /* @var $journalDao JournalDAO */
        $genreDao = DAORegistry::getDAO('GenreDAO'); /* @var $genreDao GenreDAO */
        $submissionDao = DAORegistry::getDAO('SubmissionDAO'); /* @var $submissionDao SubmissionDAO */
