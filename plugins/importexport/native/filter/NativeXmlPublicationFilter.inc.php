...
class NativeXmlPublicationFilter extends NativeXmlPKPPublicationFilter
{
    ...
    public function populateObject($publication, $node)
    {
        ...
        if (!$datePublished && ($issue || $issue_identification->length)) {
            ...
            $deployment->addError(ASSOC_TYPE_PUBLICATION, $publication->getId(), __('plugins.importexport.native.import.error.publishedDateMissing', ['publicationTitle' => $titleNodes->item(0)->textContent]));
        }
